...
def listar_ventas(request):
    query_service = get_venta_query_service()
...
def mis_ventas(request):
    q = get_venta_query_service()
    return success_response({"items": q.listar_ventas_por_usuario(request.usuario_actual.id)})


@require_GET
@require_auth()
def detalle_venta(request, venta_id: int):
    data = get_venta_query_service().obtener_venta(venta_id)
    if data is None:
        return error_response("Venta no encontrada.", status=404)
    u = request.usuario_actual
    if u.rol not in ("admin", "empleado") and data["usuario_id"] != u.id:
        return error_response("No tienes permisos para ver esta venta.", status=403)
