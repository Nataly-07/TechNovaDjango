...
class ComprasApiJwtTestCase(TestCase):
    ...
    def test_alias_api_auth_login(self):
        response = self.client.post(
            "/api/auth/login/",
            data=json.dumps({"correo_electronico": "admin.compras@demo.com", "contrasena": "123456"}),
            content_type="application/json",
        )
        self.assertEqual(response.status_code, 200)
        self.assertIn("access", response.json()["data"])

    def test_listar_compras_admin(self):
        token = self._token("admin.compras@demo.com")
